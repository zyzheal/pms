...
def cmd(request):  ##命令行
    ...
    if request.method == 'POST':
        ...
        for i in obj:
            try:
                s = ssh(ip=i.in_ip, port=i.port, username=i.system_user.username, password=i.system_user.password,
                        cmd=cmd)
                historys = history.objects.create(ip=i.in_ip, root=i.system_user, port=i.port, cmd=cmd, user=user)
                if s == None or s['data'] == '':
                    s={}
                    s['ip']=i.in_ip
