...
def index_data(request):
    ...
    content_history = Content.objects.raw(
        'SELECT b.id,count(*) as count,b.name as name FROM content_content as a  LEFT JOIN content_type as b on a.type_id=b.id  WHERE YEAR(ctime)=YEAR(CURDATE()) GROUP BY type_id')
...
def select_data(request):
    ...
    if request.method == "POST":
        if request.POST.get('type') == "project":
            ...
            if project == "All":
                sql = "SELECT b.id,count(*) as count,b.name as name FROM content_content as a  LEFT JOIN content_type as b on a.type_id=b.id  WHERE YEAR(ctime)=YEAR(CURDATE())  GROUP BY type_id"
            else:
                project_id = Project.objects.filter(name=project).values('id')[0]['id']
                sql = "SELECT b.id,count(*) as count,b.name as name FROM content_content as a  LEFT JOIN content_type as b on a.type_id=b.id  WHERE YEAR(ctime)=YEAR(CURDATE()) and project_id=%s GROUP BY type_id" % project_id
